...
for member_idx in range(n_ensemble_members):
    ...
    if use_adversarial_training:
        train_loss, val_loss = deep.train.adversarial_training_loop(model=model, 
...
mixture_test = deep.pred.compute_mixture_bergamma(x_data=x_test_stand, models=ensemble_models,
                                                                   device=device, var_target='pr', mask=y_mask,
                                                                   batch_size=16)
mixture_test.to_netcdf(f'{PREDS_PATH}/vit_{uncertainty_approach}_mixture.nc')

# Compute predictions
pred_test = deep.pred.compute_preds_mixture_ber_gamma(x_data=x_test_stand, models=ensemble_models,
